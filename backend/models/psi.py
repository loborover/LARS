...
class PsiDailyRecord(SQLModel, table=True):
    __tablename__ = "psi_daily_records"

    id: Optional[int] = Field(default=None, primary_key=True)
    part_number: str = Field(index=True)
    record_date: date = Field(index=True)
    incoming_qty: float = Field(default=0.0)   # 입고수량
    defect_qty: float = Field(default=0.0)     # 불량수량
    ...
    __table_args__ = (
        UniqueConstraint("part_number", "record_date", name="uq_psi_daily"),
    )
