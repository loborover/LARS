...
Private Sub AR_1_EssentialDataExtraction() ' AutoReport ÃÊ¹Ý ¼³Á¤ / ÇÊ¼ö µ¥ÀÌÅÍ ¿µ¿ª¸¸ ÃßÃâÇÔ
    ...
    MergeDateTime_Flexible ws, CritCol(1), 1, CritCol(2), , "ÅõÀÔ" & vbLf & "½ÃÁ¡", "d-hh:mm"
    CritCol(2) = 0
    ...
    CritCol(1) = ws.Rows(1).Find("°èÈ¹ ¼ö·®", lookAt:=xlWhole, LookIn:=xlValues).Offset(0, 1).Column
    CritCol(2) = ws.Cells(1, ws.Columns.Count).End(xlToLeft).Column
    PartCombine ws.Range(ws.Cells(1, CritCol(1)), ws.Cells(1, CritCol(2))), vStart, vEnd ' ´ÜÀÏ ÆÄÆ®¸íÀ¸·Î º´ÇÕÃ³¸®
    DeleteDuplicateRowsInColumn (CritCol(1) - 4), vStart, vEnd, ws ' WorkOrder Áßº¹ Çà Á¦°Å
    For Each vCell In ws.Range(ws.Cells(1, CritCol(1)), ws.Cells(1, CritCol(2))) ' Á¦¸ñ ¿­ ¼öÁ¤
...
Private Sub Replacing_Parts(ByRef RangeTarget As Range) ' RpP
    ' ¹üÀ§ ³» °¢ ¼¿¿¡¼­ Vender¿Í Parts Á¤º¸¸¦ ÃßÃâ ¹× Á¤¸®ÇÏ¿© ¼¿ °ªÀ» Àç±¸¼ºÇÕ´Ï´Ù.
    ...
    For Each vCell In RangeTarget
        If Len(vCell.Value) > 0 Then ' ºó ¼¿ ¹«½Ã
            ...
            Do
                ...
                sVender = ExtractBracketValue(Target, Searching) ' °ýÈ£ ¾ÈÀÇ º¥´õ ÃßÃâ
               
                 ' ¼¿°ªÀÇ Char°¡ 1ÀÚ ¹Ì¸¸ Or ¼¿°ªÀÇ Char°¡ 5ÀÚ ÀÌ»óÀÌ¸é¼­ µ¿½Ã¿¡ ¿µ¹® º¥´õÀÏ °æ¿ì = "µµÀÔÇ°"À¸·Î ´ëÃ¼
                 If Len(sVender) < 1 Or (Len(sVender) >= 5 And Not sVender Like "*[°¡-ÆR]*") Then sVender = "µµÀÔÇ°"
                 ...
                 If Not Duplicated Then vVender.Add sVender, sVender ' Áßº¹ ¾Æ´Ï¸é º¥´õ µî·Ï
                 ...
                 On Error Resume Next ' Å° Áßº¹ ¿À·ù ¹æÁö
                     ...
                     Else
                         sParts = vParts(sVender) & "/" & sParts ' º´ÇÕ
            ...
            'If ¿¹¿ÜÃ³¸® Then
                If ws.Cells(1, vCell.Column).Value = "Burner" Then
                    ...
                    Case Is = "[] 8606/8706 [±â¹Ì] 7906/8506" ' Old Gas Model
                        Target = "[Old] Better"
                    Case Else ' ¿¡·¯Ã³¸®
