...
/// <summary>
/// iC 관련 파일을 찾고, 가공(처리)하는 역할을 담당하는 클래스
...
public class iCProcessor
{
    ...
    public iCProcessor(string sourceDirectory)
    {
        _sourceDirectory = sourceDirectory;
    }

    /// <summary>
    /// 소스 폴더에서 iC 후보 파일들을 검색합니다.
    /// 예: 파일명에 "Excel_Export_" 가 포함된 xlsx 파일
    /// </summary>
    public IEnumerable<DailyPlanFile> FindiCFiles()
    {
    ...
    public void ProcessSingle(DailyPlanFile dp)
    {
        ...
        // TODO: 여기서 엑셀 열고, 기존 VBA AutoReport_iC 로직을 C#으로 옮길 예정
        Console.WriteLine($"[가공 완료] {dp.Path}");
    }

    /// <summary>
    /// 소스 폴더의 iC 파일들을 전부 순회하면서 가공합니다.
    /// </summary>
    public void ProcessAll()
    {
        foreach (var dp in FindiCFiles())
        {
            ProcessSingle(dp);
        }
    }
